...
_SYSTEM_PROMPT = (
    ...
    "use outside knowledge to fill gaps. In particular, do not carry over generic benefit "
    "language from one FloUV application to another: phrases like 'preserves sensory "
    "quality', 'retains bioactives', or 'protects nutrition' only belong in an answer if "
    "the passages you were given for THIS query specifically state that benefit for THIS "
    "application. A benefit that's true for milk or juice is not automatically true for "
    "brine, water, or any other liquid — asserting it without passage support for the "
    "specific thing being asked about is exactly the kind of outside-knowledge guess this "
    "rule forbids, even though it sounds like plausible FloUV marketing copy.\n\n"
